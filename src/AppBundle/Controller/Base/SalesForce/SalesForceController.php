...
class SalesForceController extends FOSRestController
{
    ...
    public function SFCallback(Request $request)
    {
        try {
            ...
            $result = $this->container->get('salesforce_service')->ConnectToSalesforce($code, $client_id, $client_secret, $redirect_uri);
            return $this->render('AppBundle:SalesForce:OauthTokens.html.twig', array("RefreshToken" => $result->getRefreshToken()));
        } catch (\Exception $exception) {
            throw $exception;
        }
    }
    ...
    public function FetchAction(Request $request)
    {
        try {
            ...
            $result = $this->container->get('authentication_token')->ValidateSFHeaders($request,null);
            ...
            // Fetch customers
            if ($result) {
                $response = $this->container->get('salesforce_service')->FetchCustomers($result, $fromDate, $toDate);
                return $response;
            }
        } catch (\Exception $exception) {
            throw $exception;
    ...
    public function NewTokenAction(Request $request)
    {
        try {
            // Validate the headers.
            $result = $this->container->get('authentication_token')->ValidateSFHeaders($request,GeneralSFConstants::NEW_TOKEN);
